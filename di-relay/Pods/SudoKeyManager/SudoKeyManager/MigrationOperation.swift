...
///     protocol.
/// - unhandledSudoKeyManagerError: Indicates that the operation encountered an
///     expected error from `SudoKeyManager` API.
...
public enum MigrationOperationError: Error, Equatable {
    ...
    case unhandledSudoKeyManagerError(cause: SudoKeyManagerError)
    case fatalError
}
...
public class SimpleMigrationOperation: Operation, MigrationOperation {
    ...
    /// `SudoKeyManager` instance, search parameters and the set of updates to perform on keys.
    ///
    /// - Parameters:
    ///   - version: Operation version.
    ///   - name: Operation name.
    ///   - description: Operation description.
    ///   - keyManager: `KeyManager` instance to use for migration.
    ///   - searchParams: Search parameters to use for matching keys to migrate.
    ///   - updates: Updates to apply to keys.
    public init(version: Int, name: String, description: String, keyManager: SudoKeyManager, searchParams: KeyAttributeSet, updates: KeyAttributeSet) {
    ...
    public func execute() {
        ...
        } catch let error as SudoKeyManagerError {
            self.error = MigrationOperationError.unhandledSudoKeyManagerError(cause: error)
        } catch {
