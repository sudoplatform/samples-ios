...
class Authenticator {
    ...
    func doFSSOSignOut(from presentationAnchor: ASPresentationAnchor) async throws {
        ...
        try await Clients.userClient!.presentFederatedSignOutUI(presentationAnchor: presentationAnchor)
        // After federated sign out succeeds, perform global sign out.
        try await client.globalSignOut()
        // If global sign out succeeds, we still have to launch this `ASWebAuthenticationSession` to really
    ...
    func registerAndSignIn(from presentationAnchor: ASPresentationAnchor,
